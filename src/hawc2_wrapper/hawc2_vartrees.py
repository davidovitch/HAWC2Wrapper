...
class HAWC2Aero(VariableTree):
    ...
    dynstall_method = Enum(3, (0, 1, 2, 3), desc='BEM induction method, 0=none, 1=stig oeye method,2=mhh method,3=ATEFlap')   
    atef_Ais = Array([0.00, 0.00, 0.00], desc='indicial response parameters for ATEFlap  - As')                               
    atef_Bis = Array([0.0455, 0.30, 0.30], desc='indicial response parameters for ATEFlap  - Bs')                             
    flap_in = Float(59.5925, desc='flap start (inboard)')                                                                     
    flap_out = Float(85.5023, desc='flap end (outboard)')                                                                     
    ds_filename = Str('Flap_dturwt1_Thk24')                                                                                   
    ae_sets = List([1, 1, 1])
    ae_filename = Str()
    pc_filename = Str()
